...
function slideshow(){
        ...
        var timer =setInterval(function(){
            inow++;
            tab()
            var color=$(".sensor").eq(inow).css("background-color");
          
            $(".logo-bottom-menu-head .hov1").css({"background-color":color,"border-color":color});
        },2500)
        function tab(){
            if(!aimgs){
                aimgs=$(".sensor")
        ...
        $(".logo-bottom-menu-head").on("mouseenter","span",function(){
            ...
            $(this).addClass("hov1").css({"background-color":color,"border-color":color});
            var a=$(this).index();
            $(".logo-bottom-list-min").eq(a).css("display","block");
            if(a==0){
                $(".logo-bottom-menu-head span").eq(1).css({
                    "background-color":"#fff",
                    "border-color":"#fff"
                });
            }else{
                $(".logo-bottom-menu-head span").eq(0).css({
                    "background-color":"#fff",
                    "border-color":"#fff"
                });
            }
